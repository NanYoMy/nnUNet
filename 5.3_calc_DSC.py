...
for i in methods:
    DSC[i] = {label: [] for label in range(1, 7)}  # Initialize lists for 6 labels
    IOU[i] = {label: [] for label in range(1, 7)}
            ...
            for pred_path, gd_path in zip(pred_files, gd_files):
                try:
                    ...
                    for label in labels:
                        ...
                        DSC[i][label].append(dsc_val)
                        IOU[i][label].append(iou_val)
...
# Calculate and print mean DSC and IOU for each prediction type and label
for i in methods:
    print(f"Results for {i}:")
    for label in range(1, 7):
        if DSC[i][label]:
            mean_dsc = np.mean(DSC[i][label])
            mean_iou = np.mean(IOU[i][label])
            print(f"  Label {label}:")
